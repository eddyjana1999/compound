...
class StorePurchaseService implements PurchaseService {
  ...
  Future<PurchaseOutcome> buy() async {
    try {
      ...
      // No `finally` clearing _pending here. In an async function `finally`
      // runs when the return is *evaluated*, not when the returned future
      // completes — so it nulled _pending before the purchase stream could
      // ever reach it, and every purchase hung until this timeout. _finish
      // clears it instead.

...
- **Privacy policy** — `docs/privacy.html`. Push to GitHub and enable Pages from the `docs` folder;
  the URL both stores need is then `https://<user>.github.io/<repo>/privacy.html`.
- **Screenshots** — `store/screenshots/6.9-inch` and `6.7-inch`, generated by
  `integration_test/store_screenshots_test.dart` so they can be regenerated after any UI change.
- **Listing copy** — `store/listing.md`, English and Hebrew, every field checked against its
  character limit in code.
- **App icon** — drawn, vector source under `assets/icon/source`.
- **Store name**: "Compound — Investment Calculator" is 32 characters and Apple's limit is 30, so
  the listing uses "Compound — Investment Calc". The name under the icon on the phone stays the
  short "Compound"; that one is `CFBundleDisplayName`, and a long name gets truncated there.
